...
function help_index {
    ...
    echo "Usage: sh short_read_connector_linker.sh index -b read_file [OPTIONS]"
...
function help_query {
    ...
    echo "Usage: sh short_read_connector_linker.sh -i indexed_file -q read_file_of_files [OPTIONS]"
    echo  "MANDATORY:"
...
if [ "$feature" == "index" ]; then
    ...
    # create the index
    cmd="${BIN_DIR}/SRC_linker_ram  -make_index -dumped_quasi_dict ${index_name} -graph ${out_dsk} -bank ${bank_set} -fingerprint_size ${fingerprint_size} -core ${core_used} -gamma ${gamma}  ${keep_low_complexity_option}"
...
if [ "$feature" == "query" ]; then
    ...
    cmd="${BIN_DIR}/SRC_linker_ram -dumped_quasi_dict ${index_name} -query ${query_set} -out ${result_file} -kmer_threshold ${kmer_threshold}  -core ${core_used} ${commet_like_option} ${zerod_option} ${keep_low_complexity_option} -windows_size ${windows_size}"
