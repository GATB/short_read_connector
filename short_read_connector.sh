...
#    echo  "   -d: use disk over RAM (slower and no impact with -c option)"
    ...
    echo  "            If a read of length 80 has a kmer-span of 60 with another read (of unkonwn size) from the bank, then the percentage of shared kmer span is 75%. If a least a windows (of size \"windows_size\") contains at least kmer_threshold percent of positionf covered by shared kmers, the read couple is output."
